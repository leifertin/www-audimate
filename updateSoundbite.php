...
if (($_SERVER['HTTP_REFERER']) == ('audiMate.updateSoundbite')){
...
$upMime = mysql_real_escape_string($upMime); 
//begin the query 
...
if ($rows<=0 ){ 
	...
	$sqlB = 'SELECT * FROM `soundbitesTable` WHERE `alias` = \''.$myAlias.'\' LIMIT 1;';
	$sqlRes = mysql_query($sqlB);

	$rowsz = mysql_num_rows($sqlRes); 
	if ($rowsz<=0 ){ 
		echo $myAlias." is a user but does not exist in soundbites.";
	} else { 
	
		$dbQuery = 'UPDATE soundbitesTable SET `soundbiteContent` = \''.$fileContent.'\' WHERE `alias` = \''.$myAlias.'\' LIMIT 1;';
		mysql_query($dbQuery) or die("Couldn't add file to database");
		
		$dbQuery = 'UPDATE soundbitesTable SET `mime` = \''.$upMime.'\' WHERE `alias` = \''.$myAlias.'\' LIMIT 1;';
		mysql_query($dbQuery) or die("Couldn't add file to database");
